...
func TestGetMergesLegacyAndGoRecords(t *testing.T) {
	legacy := http.HandlerFunc(func(w http.ResponseWriter, _ *http.Request) {
		_ = json.NewEncoder(w).Encode([]map[string]any{{"client_event_id": "legacy"}})
	})
	handler := Handler{Store: storeStub{record: map[string]any{"client_event_id": "go"}}, Legacy: legacy}
	...
	if err := json.Unmarshal(res.Body.Bytes(), &records); err != nil || len(records) != 2 {
